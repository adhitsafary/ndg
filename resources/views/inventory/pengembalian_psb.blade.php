    ...
    <div class="container mt-4">
        <div class="card">
            <div class="card-header bg-primary text-white" style="font-weight: 700">
                Pengembalian Inventory - PSB {{ $perbaikan->nama_plg }} - Pada Tanggal {{ $perbaikan->created_at }}
            </div>
            <div class="card-body">
                <form action="{{ route('inventory.processReturn_psb', $perbaikan->id) }}" method="POST">
                    @csrf
                    ...
                    <a href="{{ route('rekap_pemasangan.index') }}" class="btn btn-secondary mt-3">Batal</a>
                </form>
            </div>
        </div>
    </div>
