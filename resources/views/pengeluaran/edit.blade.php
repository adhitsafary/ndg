...
<div class="container mt-4">
    ...
    <form action="{{route('pengeluaran.update', $pengeluaran->id)}}" method="POST">
        ...
        <input type="number" name="harga_satuan" id="harga_satuan" value="{{$pengeluaran->harga_satuan}}" class="form-control" required oninput="hitungTotal()">

        <!-- Input Volume -->
        <label for="volume" class="mt-2">Volume</label>
        <input type="number" name="volume" id="volume" value="{{$pengeluaran->volume}}" class="form-control" required oninput="hitungTotal()">

        <!-- Input Harga Total (otomatis dihitung) -->
        <label for="harga_total" class="mt-2">Harga Total</label>
        <input type="number" name="harga_total" id="harga_total" value="{{$pengeluaran->harga_total}}" class="form-control" required readonly>
        ...
        <select name="kategori" class="form-control">
            ...
            <option value="Perlengkapan" {{$pengeluaran->kategori == 'Perlengkapan' ? 'selected' : ''}}>Perlengkapan</option>
            ...
            <option value="Jasa Perbaikan" {{$pengeluaran->kategori == 'Jasa Perbaikan' ? 'selected' : ''}}>Jasa Perbaikan</option>
            <option value="Pemasangan" {{$pengeluaran->kategori == 'Pemasangan' ? 'selected' : ''}}>Pemasangan</option>
        </select>
