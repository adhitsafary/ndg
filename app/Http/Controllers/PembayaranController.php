...
class PembayaranController extends Controller
{
...

    {
        ...
        // Filter untuk bulan dan tahun saat ini (bulan dan tahun sekarang)
        $query->whereMonth('tanggal_pembayaran', Carbon::now()->month); // Filter bulan ini
        $query->whereYear('tanggal_pembayaran', Carbon::now()->year);  // Filter tahun ini

        $queryfull = BayarPelanggan::query();
        $queryfull->orderBy('created_at', 'desc');
        ...
        // Filter berdasarkan rentang tanggal (pastikan format tanggal valid)
        if ($date_start && $date_end) {
        ...
        // Cek apakah ada filter rentang tanggal
        $gunakanQueryFull = ($date_start && $date_end);

        // Ambil hasil query sesuai kondisi
        $pembayaran = $gunakanQueryFull
            ? $queryfull->paginate(1500)->appends($request->all())
            : $query->paginate(1500)->appends($request->all());

        // Hitung total jumlah pembayaran dan total pelanggan
        $totalJumlahPembayaran = $gunakanQueryFull
            ? $queryfull->sum('jumlah_pembayaran')
            : $query->sum('jumlah_pembayaran');
