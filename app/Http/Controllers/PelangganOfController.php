...
class PelangganOfController extends Controller
{
    ...
    public function aktifkan_pelanggan3($id)
    {
        $pelangganof = Pelangganof::find($id);
    ...
    public function aktifkan_pelanggan($id)
    {
        ...
        if ($pelangganof) {
            try {
                ...
                // Hapus data dari tabel 'plg_off' setelah berhasil dipindahkan
                $pelangganof->delete();

                // Redirect dengan pesan sukses
                return redirect()->route('pelangganof.index')->with('success', 'Pelanggan OFF berhasil diaktifkan kembali.');
            } catch (\Exception $e) {
